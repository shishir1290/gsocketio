...
func Encode(p *Packet) ([]byte, error) {
	if p == nil || !p.Type.Valid() {
		return nil, fmt.Errorf("packet: unknown type")
	}
	ns := p.Namespace
	if ns == "" {
		ns = "/"
	...
	if ns != "/" {
		if !strings.HasPrefix(ns, "/") {
			return nil, fmt.Errorf("packet: invalid namespace")
		}
		b.WriteString(ns)
		b.WriteByte(',')
	}
	if p.ID != nil {
		if *p.ID < 0 {
			return nil, fmt.Errorf("packet: negative ack id")
		}
...
func Decode(raw []byte) (*Packet, error) {
	...
	if strings.HasPrefix(rest, "/") {
		...
		if idx < 0 {
			// A namespace-only CONNECT packet is valid when no payload follows.
			p.Namespace = rest
			return p, nil
		}
	...
	if i > 0 {
		id64, err := strconv.ParseInt(rest[:i], 10, 0)
		if err != nil || id64 < 0 {
			return nil, invalid(s, "ack id out of range")
		}
		id := int(id64)
		p.ID = &id
		rest = rest[i:]
...
func EventName(data json.RawMessage) (string, error) {
	...
	var name string
	if err := json.Unmarshal(a[0], &name); err != nil {
		return "", fmt.Errorf("packet: event name is not a string: %w", err)
	}
	if name == "" {
		return "", errors.New("packet: empty event name")
	}
	return name, nil
}
